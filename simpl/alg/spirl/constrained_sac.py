...
class ConstrainedSAC(ToDeviceMixin, nn.Module):
    ...
    def step(self, batch_size):
        ...
        dists, dists_value = self.policy.dist_with_value(batch.states)
        ...
        # logprob = self.policy.compute_logprob(batch.states.detach(), batch_reconsitution_states[:,0:4])
        logprob = self.policy.compute_logprob(batch.states.detach(), batch_reconsitution_states[:, 0:60])
        ...
        policy_loss = - min_qs.mean(0) + self.alpha * kl + vq_loss

        self.policy_optim.zero_grad()
        policy_loss.backward()
        self.policy_optim.step()

        stat['vq_loss'] = vq_loss
        stat['org_policy_loss'] = - min_qs.mean(0) + self.alpha * kl
