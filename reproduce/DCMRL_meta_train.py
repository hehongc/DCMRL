...
if __name__ == '__main__':
    ...
    # parser.add_argument('-d', '--domain', default='maze.simpl_meta_train_20t', type=str)
    parser.add_argument('-g', '--gpu', required=True, type=int, default=0)
    parser.add_argument('-w', '--worker-gpus', required=True, type=int, default=0,nargs='+')
    parser.add_argument('-s', '--spirl-pretrained-path', required=True, default='/home/hehongcai/code/lib/SiMPL/reproduce/test/spirl_pretrained_maze.pt', type=str)

    parser.add_argument('-t', '--policy-vis_period', type=int)
    parser.add_argument('-p', '--wandb-project-name', default='SiMPL', type=str)
    parser.add_argument('-r', '--wandb-run-name', default='test_maze_20_hhc', type=str)
    parser.add_argument('-a', '--save_file_path', default='/home/hehongcai/code/lib/SiMPL/reproduce/test/Save/Maze_20/Meta_training_Maze_20.pt', type=str)
    ...
    warm_time = warm_end_time - warm_start_time
    print("warm_time_for_building_buffer: ", warm_time)
    ...
    for epoch_i in range(1, config['n_epoch']+1):
        ...
        if epoch_i >=400 and epoch_i % 100 == 0:
            current_save_filepath = save_filepath[:-3] + '_' +str(epoch_i) + '.pt'
            torch.save({
                'encoder': encoder,
                'high_policy': high_policy,
                'qfs': qfs,
                'policy_post_reg': trainer.policy_post_reg().item()
            }, current_save_filepath)
